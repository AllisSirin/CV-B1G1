...
import { withoutEnded } from "../public/period.mjs";
...
/** 캐시에는 읽은 그대로 두고, 내보낼 때 끝난 딜을 뺀다 — 갱신마다 지난 행사가 걸러진다 */
async function loadSource(source, refresh) {
  ...
  return withoutEnded({ ...meta(source), ...entry.value, fetchedAt: entry.fetchedAt, stale: entry.stale, error: entry.error });
}
